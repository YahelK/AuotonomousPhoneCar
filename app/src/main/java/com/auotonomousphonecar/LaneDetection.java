...
public class LaneDetection {
    ...
    public static Mat betaFrames1 = new Mat(POLY_ORDER + 1, 0, CvType.CV_32FC1);
    public static Mat betaFrames2 = new Mat(POLY_ORDER + 1, 0, CvType.CV_32FC1);
...
//                Imgproc.rectangle(out, new Point(winxLeftLow, winyLow), new Point(winxLeftHigh, winyHigh),
//                        new Scalar(100, 255, 255), 3);
//                Imgproc.rectangle(out, new Point(winxRightLow, winyLow), new Point(winxRightHigh, winyHigh),
//                        new Scalar(100, 255, 255), 3);
            }
...
//            rightLaneInds.push_back(goodRightInds); // to be deleted
        ...
        Mat beta1 = new Mat(POLY_ORDER + 1, 1, CvType.CV_32F), beta2 = new Mat(POLY_ORDER + 1, 1, CvType.CV_32F);
        if (leftLaneInds.height() > POLY_ORDER && leftLaneInds.height() > POLY_ORDER) {

            fitPoly(leftLaneInds, POLY_ORDER, true, beta1);
            fitPoly(rightLaneInds, POLY_ORDER, true, beta2);

            // calculate polynomial constants with last frames mean
            if (frameCounter < NUM_FRAMES) {
                List<Mat> src = Arrays.asList(betaFrames1, beta1);
                Core.hconcat(src, betaFrames1);
                src = Arrays.asList(betaFrames2, beta2);
                Core.hconcat(src, betaFrames2);
                frameCounter++;
            } else {
                beta1.copyTo(betaFrames1.col(frameCounter % NUM_FRAMES));
                beta2.copyTo(betaFrames2.col(frameCounter % NUM_FRAMES));
            }
        }
        Core.reduce(betaFrames1, beta1, 1, Core.REDUCE_AVG);
        Core.reduce(betaFrames2, beta2, 1, Core.REDUCE_AVG);


        // calculate steering
        double carPos = img.width() / 2;
        double laneCenterPos = (calculatePoly(img.height() - 1, beta1) +
                calculatePoly(img.height() - 1, beta2)) / 2;
        double offset = carPos - laneCenterPos;

        double lanesAngle = (calculatePolyAngle(img.height() - 1, beta1) +
                calculatePolyAngle(img.height() - 1, beta2)) / 2;

        double steeringAngle = lanesAngle + Math.atan(offset / img.height());

        // draw the road in the out image
        outImg.create(img.size(), CvType.CV_8U);
        outImg.setTo(new Scalar(0));
        fillCurves(beta1, beta2, outImg, 8);
     ...
     *
     * @param beta1 input vector of the coefficients of the polynomial left curve
     * @param beta2 input vector of the coefficients of the polynomial right curve
     * @param img   image to draw inside
     * @param step  the distance in pixels between two points of the curves to fill (speed <-> accuracy)
     */
    public static void fillCurves(Mat beta1, Mat beta2, Mat img, int step) {
        ...
        for (int i = 0, ind = 0; ind < n; i += step, ind++) {
            drawPoints1[ind] = new Point(calculatePoly(i, beta1), i);
            drawPoints2[ind] = new Point(calculatePoly(i, beta2), i);
        }
        // fill bottom of the image
        drawPoints1[n] = new Point(calculatePoly(img.rows() - 1, beta1), img.rows() - 1);
        drawPoints2[n] = new Point(calculatePoly(img.rows() - 1, beta2), img.rows() - 1);
