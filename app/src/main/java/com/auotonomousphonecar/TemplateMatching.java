...
import static com.auotonomousphonecar.ImageOperations.shapesNames;
...
//        template.matchResult.create(1, 1, CvType.CV_32FC1);
    ...
    public static void signMatching(Mat dst, Mat org, ImageOperations.Shape shape, List<SignTemplate> templates,
        ...
        float scale = dst.width()/(float)org.width();
...
//            Core.normalize(img, img, 0, 1, Core.NORM_MINMAX, -1, new Mat());
        ...
        if (matchVal >= 0.82) {
            ...
            Start = new Point(2 * r.x, 2 * r.y);
            End = new Point(2 * (r.x + r.width), 2 * (r.y + r.height));
            ...
            if(colorMode == ColorMode.COLORS) {
                ...
                Size s = r.size();
                Rect r2 = new Rect(2 * r.x, 2 * r.y, 2 * r.width, 2 * r.height);
                s.height = 2 * s.height;
                s.width = 2 * s.width;
    ...
    /**
     * Color filtering by given template
     *
