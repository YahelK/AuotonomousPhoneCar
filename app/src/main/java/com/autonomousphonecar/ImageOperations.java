...
//                Core.add(s, rgbchannels.get(2), s);
     ...
     */
    public static void shapeDetection(Mat bw, Mat dst, List<Shape> shapes, ColorName color, boolean shapeMode, ShapeName... shapesNames) {
    ...
    public static void warpDownSampleImage(Mat img) {
        ...
        Point[] srcPoints = {new Point(0, 0), new Point(img.width(), 0),
                new Point(0, img.height()), new Point(img.width(), img.height())};
        MatOfPoint2f srcMOP = new MatOfPoint2f();
        srcMOP.fromArray(srcPoints);

        Point[] dstPoints = {new Point(0, 0), new Point(newSizeW, 0),
                new Point(0, newSizeH), new Point(newSizeW, newSizeH)};
        MatOfPoint2f dstMOP = new MatOfPoint2f();
        dstMOP.fromArray(dstPoints);

        Mat M = Imgproc.getPerspectiveTransform(srcMOP, dstMOP);

        Imgproc.warpPerspective(img, img, M, new Size(newSizeH, newSizeW));
     ...
     */
    public static Mat setRotationMatrix(float theta, String axis) {

        Mat rotMat = Mat.zeros(3, 3, CvType.CV_64FC1);

        switch (axis) {
            case "x":
                rotMat.put(0, 0, 1);
                rotMat.put(1, 1, Math.cos(theta));
                rotMat.put(2, 1, Math.sin(theta));
                rotMat.put(1, 2, -Math.sin(theta));
                rotMat.put(2, 2, Math.cos(theta));
                break;
            case "y":
                rotMat.put(1, 1, 1);
                rotMat.put(0, 0, Math.cos(theta));
                rotMat.put(0, 2, Math.sin(theta));
                rotMat.put(2, 0, -Math.sin(theta));
                rotMat.put(2, 2, Math.cos(theta));
                break;
            case "z":
                rotMat.put(2, 2, 1);
                rotMat.put(0, 0, Math.cos(theta));
                rotMat.put(1, 0, Math.sin(theta));
                rotMat.put(0, 1, -Math.sin(theta));
                rotMat.put(1, 1, Math.cos(theta));
                break;
        }
...
//    }
     ...
     * Make a histogram of each column in the image
     *
     * @param val  the input gray scale image
     */
